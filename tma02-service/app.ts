...
class Photo {
    ...
    constructor(user:string, id:string, location:string, uri:string) {  // Constructor
        this.user = user;
        this.id = id;
 ...
 */
app.get(base, (request:any, response:any) => {
 ...
 * @param response  the response object
 */
app.get(base+":id", (request:any, response:any) => {
    if (request.query.userid === undefined || request.query.userid === "" || !isUserRegistered(request.query.userid)){
        response.send("{\"status\" : \"error\",\"message\" : \"User not registered\"}");
    }
    else {
 ...
 * @param response  the response object
 */
app.post(base, (request:any, response:any) => {
 ...
 * @param response  the response object
 */
app.post(base+"comment/:id", (request:any, response:any) => {
    if (request.body.userid === undefined || request.body.userid === "" || !isUserRegistered(request.body.userid)){
        response.send("{\"status\" : \"error\",\"message\" : \"User not registered\"}");
    }
    else {
        response.send(postCommentImp(request.params.id.toString(),
            request.body.userid+": "+request.body.comment));   
    }
});
 ...
 * @param response  the response object
 */
app.post(base+"vote/:id", (request:any, response:any) => {
    if (request.body.userid === undefined || request.body.userid === "" || !isUserRegistered(request.body.userid)){
        response.send("{\"status\" : \"error\",\"message\" : \"User not registered\"}");
    }
    else {
...
function getImp(id:string): string{
    ...
    let result:Photo[] = [];
    photos.forEach(function (photo: Photo){
 ...
 */
function postImp(userid:string, location:string, photo:string): string{
...
function isUserRegistered(userid:string):boolean{
    let found = false;
    users.forEach(function (user: string){
